...
var getJson = external_fansion_base_default.a.rest.getJson,
    depend = external_fansion_base_default.a.mod.depend;

var templates = {};

var templates_getMeta = function getMeta(path, metas) {
...
/* harmony default export */ var src_templates = ({
  ...
  rule: function rule(path, metas) {
    return path ? function (_) {
      return templates_getMeta(path, metas);
    } : null;
  }
});
...
var addMeta = external_fansion_base_default.a.builder.register(metas_metas, 'fac-name');
...
function fac_getLayout(meta) {
  var name = meta.layout;
  var exist = !!name;
  if (exist && typeof name !== 'string') {
    return assign_default()({ exist: exist }, name);
  }
  return { exist: exist, name: name };
}

function fac_getMeta(vm) {
  var meta = typeof vm.meta === 'string' ? src_metas.getMeta(vm.meta) : vm.meta;
  ...
  var resolve = once(function (res) {
    ...
    if (typeof res.uimeta === 'string') {
      try {
        fac_getJson(res.uimeta).then(function (meta) {
          res = assign_default()(res, meta);
        });
      } catch (e) {
        console.log(e);
      } finally {
  ...
  if ((typeof meta === 'undefined' ? 'undefined' : typeof_default()(meta)) === 'object' && typeof meta.then === 'function' && !factory.resolved) {
...
/* harmony default export */ var fac = ({
  ...
  beforeMount: function beforeMount() {
    var _self = this;
    var meta = fac_getMeta(_self);
    if (!meta || _self.metaing) {
      return;
    }
    _self.meta = meta;

    var layout = fac_getLayout(meta);

    var _ref = meta.noc ? meta : comps["a" /* default */].compileComps(meta.components),
        components = _ref.components,
        template = _ref.template;

    assign_default()(_self, meta.methods, { layout: layout.meta }, typeof meta.member === 'function' ? meta.member.call(this) : meta.member);

    var metaData = typeof meta.data === 'function' ? meta.data.call(this) : meta.data;

    if (metaData !== null && (typeof metaData === 'undefined' ? 'undefined' : typeof_default()(metaData)) === 'object') {
      var old = _self._data.__ob__.vmCount;
      _self._data.__ob__.vmCount = 0;
      entries_default()(metaData).forEach(function (_ref2) {
    ...
    if (layout.exist) {
      var layoutComp = src_layouts.get(layout.name);
      fac_toRender(this, '<layout :conf="layoutConf" v-loading="pageLoading">' + template + '</layout>', assign_default()({ layout: layoutComp }, components));
    } else {
...
var src_pageMetaRule = function pageMetaRule(path, pageMetas) {
  if (path.startsWith('$')) {
    var meta = { props: { config: path.substring(1) }, component: fac };
...
var src_install = function install(Vue) {
  ...
  external_fansion_base_default.a.init({ page: { rules: src_pageMetaRule } });
  src_init(options);
  src_init(opts);
};
